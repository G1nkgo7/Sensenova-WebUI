---
name: sn-ppt-web-zh
description: 创建或编辑完整的 HTML 幻灯片 deck（快速版，新建流程不做全册终审 Review）；支持主题、提纲、文档和多附件输入，生成每页独立 HTML（1600×900）、渲染图、逐页讲稿与播放器。新建时编排 Research、Material、Image、Slide 后直接交付（跳过 final_review）；编辑现有 PPT 时按复杂度选择单 Review 快修或 Orchestrator 多 Agent 改造。适用于快速制作、改稿、续编、重排、统一风格 PPT、deck、slides、presentation。
---
...
| Review | **仅编辑流程使用**（本快速版新建流程不派 Review）；编辑时首次验收 1 个、修复后最多复验 2 次，简单编辑时也是执行者 | 先诊断、后集中修复、批量重渲和最终讲稿收口 |
...
后续主链只有一条：`Style Lock → 全册计划 + prepare → Image 分片并行 → 素材路径一次回填 → Slide 页组并行 → 讲稿同步 + build → 交付`（**本快速版不含 Review 环节**）。前一节点的真相源未冻结，不启动依赖它的下游；互不依赖的同层任务一次并行派出。`build` 可能裁剪字体、更新 `base.css` 并重渲全册。
...
5. 等待全部页面完成后直接进入阶段 5 交付（**本快速版新建流程不派 Review**）。Orchestrator 不追逐 `cjkTypography`、`crowded`、bbox/contrast 候选、轻微换行/标点等 advisory，也不开启审美清门循环。页面质量在 Slide 阶段一次做扎实；确有硬伤应在 Slide 页组内解决，不在交付阶段引入 Review。

### 阶段 5：讲稿同步与交付（本版跳过全册 Review）

**本 skill 是「快速版」：新建流程不委派 final_review 子 agent、不做全册像素收口。**页面由各 Slide 页组产出后即视为定稿；Orchestrator 不再派 Review 诊断/返修，也不在交付前逐页看图。目标是显著提速与省 token，代价是少一道视觉自检——请在 Slide 阶段就把每页做扎实。

Slide 页组全部完成后，Orchestrator 直接收口交付：

1. **同步讲稿**：为每页写自然、可朗读的 `## 口语讲稿` 到 `plan/slide_NN.md`（不机械枚举屏显标签，不泄漏内部规划字段/来源/文件名）。
2. **确定性 build**：执行一次
...
```
   ...
   生成最终 `base.css`/字体裁剪、全册渲染与 `present.html`/交付包。
3. **交付**：确认全部 slide、非空最终渲染、`speech.md` 与可打开的 `present.html`/交付包存在后返回 `ready`。缺页、渲染缺失/空白、播放器或交付包无法构建/打开等不可用技术故障才判失败。

不派 Review 子 agent；不为审美偏好开启额外看图循环。若确有明显硬伤，属于 Slide 阶段该解决的问题，不在本阶段引入 Review。
