...
- **Whole-deck font families ≤ 3, applied uniformly — no per-page reshuffling.** Across the entire deck use at most 3 font families (typically: one Chinese title/body family + one Latin/numeric family + at most one on-topic accent family); the same role uses the same family on every page. Formal decks converge to 1–2 families. More roles never permit mixed families inside one Chinese phrase. Body, tables, chart labels, footers, and eyebrows use `--font-sans`; hero/KPI/section numbers use `--font-number` per Style Lock; **never use `--font-mono` for Chinese** (no CJK glyphs → cartoon-font fallback) — `--font-mono` is only for pure-Latin code, coordinates, and IDs. For a modern editorial or experimental direction, Unbounded or Syne may carry Latin display, Archivo may carry numbers and compact Latin labels, and Noto Sans SC should carry Chinese and body copy.
