---
name: sn-ppt-web-en
description: Create or edit complete HTML slide decks; supports themes, outlines, documents, and multi-attachment inputs, generating independent HTML per page (1600×900), rendered images, page-by-page presentation scripts, and a player. For new creations, orchestrates Research, Material, Image, Slide, and Review; for editing existing PPTs, selects either single Review quick-fix or Orchestrator multi-agent overhaul based on complexity. Applicable for producing, revising, extending, reordering, unifying styles, or reviewing PPTs, decks, slides, and presentations.
---
...
| Review | 1 for initial acceptance, at most 2 re-verifications after fixes; also acts as the executor during simple edits | Diagnose first, then perform centralized fixes, batch re-rendering, and final presentation script finalization |
...
There is only one subsequent main chain: `Style Lock → 全册计划 + prepare → Image 分片并行 → 素材路径一次回填 → Slide 页组并行 → Review → 必要时原页组有限返修与 Review 复验 → build`. If the source of truth of the preceding node is not frozen, downstream tasks dependent on it will not be started; independent tasks at the same level are dispatched in parallel at once.
...
5. Wait for all pages to be completed before starting the first Review. During new creation or complex editing processes, do not additionally delegate `simple_edit` or `review-fix` roles; Orchestrator must not chase advisories such as `cjkTypography`, `crowded`, bbox/contrast candidates, or minor line breaks/punctuation, nor should it open aesthetic cleanup cycles before Review. When Review detects real critical flaws with fresh pixel/DOM evidence, hand them back only to the original Slide Group; each group can be reworked at most twice, and each failure will be recovered by the runtime to the last reviewed version of that group. The next Review can only be started after rework, with a maximum of 3 Reviews in total.

### Phase 5: Full-Deck Review and Delivery

The goal of each Review must start with the following language contract, followed by the specific diagnostic scope:
...
The language must not be merely implied in the parent task or system, nor should it be omitted to let Review guess on its own. Subsequently, execute strictly in two steps:

1. **Complete Diagnosis:** First view the overview, then view all contact sheets and necessary single pages in batches according to `review-contact.json`; record the covered page numbers and findings of each batch into the same `_trace/review-issues.md`. Full-deck coverage is achieved and the ledger is frozen before modifications or rendering are prohibited; do not skip subsequent batches due to a long Deck page count.
2. **Content Fidelity Verification:** When the task contains attachments or uses Research, compare the on-screen facts of each page with `grounded-knowledge.md` before pixel modification; if there are attachments, cross-reference them with the Material summary and coverage ledger, and write `_trace/content-fidelity.md`. If numbers, names, dates, units, product identities, exact quotes, or relationships cannot be traced or are self-contradictory, correct them or mark as blocked. Generated images can only bear conceptual/atmospheric expressions; if used for identifying named real products, people, or cases, the page must be clearly labeled "Conceptual Illustration" and cannot serve as factual evidence. Only when there are no attachments, no Research, and no high-risk external facts can `content_fidelity` be `not-applicable`.
   During Review stall closeout, the only canonical artifacts that may be completed or updated are `_trace/review-issues.md` and `_trace/content-fidelity.md`. The Harness must not block either artifact required by final acceptance, and it must not allow page edits during closeout.
3. **Centralized Fixes:** Review both diagnoses and directly fixes problems that can be safely resolved within the current boundary; problems that can be resolved with current files and existing assets must not be merely reported to Orchestrator. Address common root causes globally first, then locally, and perform unified batch rendering only after all modifications are completed. This entire batch of "modification → batch rendering → focus re-verification" is recorded as 1 round of refine for Review. Any HTML/`base.css` modification invalidates old PNGs, and calling Vision again before re-rendering is prohibited; Canvas/SVG/HTML overlay pages must synchronously correct CSS dimensions, Canvas attributes, SVG `viewBox`, JS coordinates, and node anchors, rather than just enlarging the outer container. Automated checks for `boxoverflow`, bbox intersections, and decorative intersections are only positioning candidates; if fresh pixels have no real occlusion, cropping, or unreadability, do not shrink text, compress subjects, or delete elements with compositional roles just to clear warnings.
4. When base.css/fonts are modified, perform a full-deck batch; when only local changes are made, perform a page batch.
5. Generate a focus contact sheet for re-verification. A single Review performs only 1 round of refine; if visible critical flaws remain, return `blocked`, and Orchestrator will return the evidenced critical flaws to the original page group. The original page group retains the last verified version, performs a merged fix, and re-renders for review; if the new version degrades or remains unresolved, restore the verified version. After fixing, start a new Review re-verification, up to a maximum of 3 Reviews, without adding new aesthetic goals.
6. Synchronize the script after pixel finalization.
7. After Review returns `ready` and has successfully built, Orchestrator only reads its structured conclusions and confirms the existence of delivery files; it must not re-render, build, view the same PNG/contact sheet, or re-diagnose the same issues. Only when new page modifications occur after Review should the next controlled Review re-verification be initiated.
When a Review times out, returns `blocked`, lacks final pixel re-verification, or has no natural return contract, enter a limited recovery workflow first instead of immediately marking the entire task as failed. Once the budget of 3 Reviews or 2 re-works per affected page group is reached, stop modifying pages, retain `_trace/review-issues.md`, restore the last verified version of each group, and execute a deterministic build. As long as all slides, non-empty final renders, `speech.md`, and an openable `present.html`/delivery package exist, the task is delivered as "Completed (with room for improvement)" accompanied by warnings; the task is marked as failed only in the case of unusable technical failures such as missing pages, missing/blank renders, or a player or delivery package that cannot be built or opened.

Review does not merely check "whether there is overflow", but also compares the design realization of the entire deck: whether the cover has a dominant focal point and necessary hierarchy; whether chapter pages possess both affinity and reflect chapter progression; whether the ending responds to the opening; whether regular pages fully utilize the canvas under the projection font scale and form a clear reading path; whether each chapter boundary still belongs to the same base canvas family; whether full-page transitions have clear entry and exit handoffs; and whether the screen display leaks internal planning fields, sources, filenames, or pseudo-metadata with no audience value. It must be blocked if `vision_analyze` has not been actually called to inspect the final pixels.

```bash
python ${SKILL_DIR:-skills/sn-ppt-web-en}/scripts/deck.py build . --expected <总页数>
```

When the Review is ready, deliver as `ready`. If the Review ultimately still has major flaws but all pages, renders, scripts, and `present.html` are usable, deliver as `needs_improvement` and display the issue ledger; usable completed drafts must not be discarded due to advisories, sub-agent text polishing, or imperfect Review contracts.
