#!/usr/bin/env bash
# ============================================================================
# long-horizon-presenter · 环境安装脚本(让 skill 自包含:一键装齐运行所需的全部依赖)
#
# 装什么(四块,缺一不可):
...
#
# 产物路径(可用环境变量覆盖):
#   NORMALIZE_VENV   默认 ~/.cache/long-horizon-presenter/venv-normalize
#   FONTS_DIR        默认 ~/.fonts
...
NORMALIZE_VENV="${NORMALIZE_VENV:-$HOME/.cache/long-horizon-presenter/venv-normalize}"
